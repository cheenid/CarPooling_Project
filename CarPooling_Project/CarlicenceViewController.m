...
@interface CarlicenceViewController ()
<UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate>
@property (nonatomic, strong) UIImageView *carLicenceImageView;
@end

@implementation CarlicenceViewController
...
- (void)initInfoLabel
{
    ...
    frontImagelabel.text = @"行驶证照片";
    [self.view addSubview:frontImagelabel];
}

- (UIImageView*)carLicenceImageView
{
    if (_carLicenceImageView == nil)
    {
        CGRect frame = CGRectMake(10, 25, KScreenWidth-20, 180);
        _carLicenceImageView = [[UIImageView alloc]initWithFrame:frame];
        _carLicenceImageView.contentMode = UIViewContentModeScaleAspectFit;
        _carLicenceImageView.layer.cornerRadius = 8.0;
        _carLicenceImageView.layer.masksToBounds = YES;
        _carLicenceImageView.layer.borderColor = [UIColor colorWithRed:0.524 green:0.533 blue:0.508 alpha:1.0].CGColor;
        _carLicenceImageView.layer.borderWidth = 2.0;
        _carLicenceImageView.userInteractionEnabled = YES;
    }
    return _carLicenceImageView;
}
...
{
    UITapGestureRecognizer *frontImageTapGesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(carLicenceImageDidTap)];
    frontImageTapGesture.numberOfTapsRequired = 1;
    [self.carLicenceImageView addGestureRecognizer:frontImageTapGesture];
}
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self initInfoLabel];
    [self.view addSubview:self.carLicenceImageView];
...
- (void)updateContentView
{
    ...
    if (personalData.carLicencesPhoto)
    {
        NSString *baseURLStr = @"http://scar.qiniudn.com/";
        NSString *urlString = [NSString stringWithFormat:@"%@%@",baseURLStr,personalData.carLicencesPhoto];
        NSURL *url = [NSURL URLWithString:urlString];
        [self.carLicenceImageView setImageWithURL:url];
    }
}
...
- (void)clickToUpload
{
    if (self.carLicenceImageView.image == nil)
    {
    ...
    //生活照图片
    NSData *avatarImageData = UIImagePNGRepresentation(_carLicenceImageView.image);
    ...
    [qiniuUploader execuploadTask:^(AFHTTPRequestOperation *operation, NSInteger index, NSDictionary *ret) {
        ...
        [weakSelf uploadCarLicenceImageData:key];
...
- (void)uploadCarLicenceImageData:(NSString*)imageKey
{
    [[YZProgressHUD progressHUD]changeHUDWithText:@"请稍后" detailText:@"驾驶证认证中"];
    [[CPHttpRequest sharedInstance]requestUploadCarlicenseVerify:imageKey success:^(id responseObject) {
...
-(void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
    ...
    self.carLicenceImageView.image = image;
}
